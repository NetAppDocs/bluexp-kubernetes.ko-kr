...
BlueXP 커넥터:: Connector는 필요한 권한을 사용하여 Azure에서 실행 중이어야 합니다.  a Connector,아래에서 자세히 알아보십시오.
...
RBAC 인증:: BlueXP는 Active Directory를 사용하는 RBAC 지원 클러스터를 지원합니다. 각 Azure 클러스터마다 BlueXP Connector 역할이 허가되어야 합니다.  up RBAC authorization,아래에서 자세히 알아보십시오.
