...
BlueXP 커넥터:: 필요한 권한이 있는 Connector가 AWS에서 실행되고 있어야 합니다.  a Connector,아래에서 자세히 알아보십시오.
...
RBAC 인증:: BlueXP Connector 역할은 각 Kubernetes 클러스터에서 인증되어야 합니다.  up RBAC authorization,아래에서 자세히 알아보십시오.
